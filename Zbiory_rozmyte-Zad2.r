...
wine_data <- read.table("C:\\Users\\Michał\\OneDrive\\Pulpit\\R\\wine.txt", header = TRUE, sep = " ")
...
avg_alcohol = fuzzy_trapezoid_gset(corners = c(12, 12.5, 13.5, 14), universe=u1)
...
alcohol_sets = list(low_alcohol=low_alcohol, avg_alcohol=avg_alcohol, high_alcohol=high_alcohol)
plot(alcohol)
...
malic_acid_sets <- list(low_malic_acid=low_malic_acid, high_malic_acid=high_malic_acid)
plot(malic_acid)
...
color_intensity_sets = list(low_color_intensity =low_color_intensity, high_color_intensity=high_color_intensity)
...
cultivar_sets = list(low_cultivar=low_cultivar, avg_cultivar=avg_cultivar, high_cultivar=high_cultivar)
plot(cultivar)
...
low_malic_acid <- malic_acid_sets[["low_malic_acid"]]

# Find the index of value malic_acid=2
low_malic_acid_idx <- which(low_malic_acid$values == 2)

# Retrieve the membership value
low_malic_acid_membership <- low_malic_acid$membership

# Display the membership value for malic_acid=2
print(low_malic_acid_membership[low_malic_acid_idx])